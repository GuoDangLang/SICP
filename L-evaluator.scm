	...
	((compound-procedure? procedure)
	 (eval-sequence
	   ...
	   (extend-environment
	     ...
	     (list-of-args (procedure-arrange procedure) arguments env)
  ...
  (if (no-operands? exps)
    ...
    (cons (actual-value (first-operand exps) env)
	  (list-of-arg-values (rest-operands exps)
			      env))))
(define (list-of-args arrange exps env)
  (define (scan arrange exps)
    (cond ((null? arrange) '())
	  ((eq? (car arrange) 'lazy) (cons (delay-it-nomemo (car exps) env)
					   (scan (cdr arrange) (cdr exps))))
	  ((eq? (car arrange) 'lazy-memo) (cons (delay-it-memo (car exps) env)
						(scan (cdr arrange) (cdr exps))))
	  ((eq? (car arrange) 'ordi) (cons (actual-value (car exps) env)
					   (scan (cdr arrange) (cdr exps))))))
  (scan arrange exps))
...
(define (make-procedure parameters body env)
  (let ((filter-parameters (map (lambda (x) (if (pair? x) (car x) x)) parameters))
	(arrangement (map (lambda (x) (if (pair? x) (cadr x) 'ordi)) parameters)))
  (list 'procedure filter-parameters body env arrangement)))
(define (compound-procedure? p)
  (tagged-list? p 'procedure))
(define (procedure-arrange p)
  (cadr (cdddr p)))
