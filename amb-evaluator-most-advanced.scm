;;;it implement require as an ordinary procedure that use amb.
;;;and with both permanent-set! and let-fail;
...
(define (analyze exp)
  (cond ((amb? exp) (analyze-amb exp))
	...
	((if-fail? exp) (analyze-iffail exp))
...
(define (if-fail-conse exp)
  (cadr exp))
(define (if-fail-alte exp)
  (caddr exp))
(define (analyze-iffail exp)
  (let ((conse (analyze (if-fail-conse exp)))
	(alte (analyze (if-fail-alte exp))))
    (lambda (env succeed fail)
      (conse
	env
	succeed
	(lambda () (alte env succeed fail))))))
