  ...
  (let ((contents '*unassigned*)
    ...
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
	    ...
	    ((eq? message 'push-reg) (lambda (value)
				       ((stack 'push) value)))
	    ((eq? message 'pop-reg) (stack 'pop))
	    ((eq? measage 'initialize) (stack 'initialize))
	    (else 
	      (error "Unkown request -- REGISTER" message))))
    dispatch))
(define (pop-regstack register)
  (register 'pop-reg))
(define (push-regstack register value)
  ((register 'push-reg) value))
  ...
  (let ((pc (make-register 'pc))
    ...
    (let  ((the-ops
	    (list (list 'initialize-stack
			(lambda () (map (lambda (pair) (initialize-reg (cadr pair))) register-table)))))
	  (register-table
...
(define (make-save inst machine stack pc)
  (let ((reg (get-register machine (stack-inst-reg-name inst))))
    (lambda ()
      (push-regstack reg (get-contents reg))
      (advance-pc pc))))
(define (make-restore inst machine stack pc)
  (let ((reg (get-register machine (stack-inst-reg-name inst))))
    (lambda ()
      (set-contents! reg (pop-regstack reg))
      (advance-pc pc))))
