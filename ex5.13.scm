...
(define (extract-labels text receive machine)
  (define (allocate-regs inst)
    (for-each (lambda (element) (if (and (not (symbol? element)) (equal? (car element) 'reg))
				  ((machine 'allocate-register) (cadr element))))
	      inst))
  (if (null? text)
    (receive '() '())
			...
			(if (symbol? next-inst)
			  (receive insts
				   (cons (make-label-entry next-inst
							   insts)
					 labels))
			  (begin (allocate-regs next-inst) (receive (cons (make-instruction next-inst)
					 insts)
				   labels)))))
		    machine)))
