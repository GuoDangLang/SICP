;*****This simulator can print number of instructions of executed procedure by type in (machine 'print-insts)*****
;Table
...
(define (make-execution-procedure inst labels machine pc flag stack ops counter)
  (cond ((eq? (car inst) 'assign)
	 (make-assign inst machine labels ops pc counter))
	((eq? (car inst) 'test)
	 (make-test inst machine labels ops flag pc counter))
	((eq? (car inst) 'branch)
	 (make-branch inst machine labels flag pc counter))
	((eq? (car inst) 'goto)
	 (make-goto inst machine labels pc counter))
	((eq? (car inst) 'save)
	 (make-save inst machine stack pc counter))
	((eq? (car inst) 'restore)
	 (make-restore inst machine stack pc counter))
	((eq? (car inst) 'perform)
	 (make-perform inst machine labels ops pc counter))
	((eq? (car inst) 'initialize)
	 (make-initialize machine pc counter))
	((eq? (car inst) 'statistics)
	 (make-statistics machine pc counter))
	(else (error "Unkown instruction type --ASSEMBLE" inst))))
;Statistics instructions
(define (make-statistics machine pc counter)
  (lambda ()
    ((machine 'stack) 'ps)
    (advance-pc pc counter)))
;Initialize instructions
(define (make-initialize machine pc counter)
  (lambda ()
    ((machine 'stack) 'initialize)
    (advance-pc pc counter)))
;Assign instructions
(define (make-assign inst machine labels operations pc counter)
  (let ((target 
...
(define (make-test inst machine labels operations flag pc counter)
...
(define (make-branch inst machine labels flag pc counter)
...
(define (make-goto inst machine labels pc counter)
  (let ((dest (goto-dest inst)))
    (cond ((label-exp? dest)
	   (let ((insts
		   (lookup-label labels (label-exp-label dest))))
	     (lambda () (set-contents! pc insts) (set-contents! counter (+ (get-contents counter) 1)))))
	  ((register-exp? dest)
	   (let ((reg
...
;Other instructions
(define (make-save inst machine stack pc counter)
  (let ((reg (get-register machine (stack-inst-reg-name inst))))
    (lambda ()
      (push stack (get-contents reg))
      (advance-pc pc counter))))
(define (make-restore inst machine stack pc counter)
  (let ((reg (get-register machine (stack-inst-reg-name inst))))
    (lambda ()
...
(define (make-perform inst machine labels operations pc counter)
