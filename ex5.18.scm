  ...
  (let ((machine (make-new-machine)))
    (for-each (lambda (register-name)
		...
		(cond ((eq? m 'trace-on) (if (machine m) (begin ((machine 'install-instruction-sequence) (assemble controller-text machine)) 'ok)))
		      ...
		      ((eq? m 'reg-trace-on) (lambda (reg) (if ((machine m) reg) (begin ((machine 'install-instruction-sequence) (assemble controller-text machine)) 'ok))))
		      ((eq? m 'reg-trace-off) (lambda (reg) (if ((machine m) reg) (begin ((machine 'install-instruction-sequence) (assemble controller-text machine)) 'ok))))
		      (else (machine m))))
	      dispatch))
