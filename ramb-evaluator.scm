...
(define (analyze-amb exp)
  (let ((cprocs (map analyze (amb-choices exp))))
    (lambda (env succeed fail)
      (define (get-list choices)
	(define (get n accu left)
	  (if (= n 0)
	    (cons (car left) (append accu (cdr left)))
	    (get (- n 1) (cons (car left) accu)
		 (cdr left))))
	(get (random (length choices)) '() choices))
      (define (try-next choices)
	(if (null? choices)
	  (fail)
	  (let ((list (get-list choices)))
	    ((car list) env
			 succeed
			 (lambda ()
			   (try-next (cdr list)))))))
